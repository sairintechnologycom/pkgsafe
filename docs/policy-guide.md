...
- `sandbox.behavior_mode`: `disabled`, `heuristic`, or `isolated`; `heuristic` is host execution without OS isolation, and `isolated` must only be used when a real isolation backend is available
