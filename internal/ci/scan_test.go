...
trusted_packages:
  npm:
    - name: "react"
      version_range: "*"
      reason: "Approved frontend framework"
`
...
exceptions:
  - id: "EXC-1"
    ...
    package: "legacy-tool"
    version_range: "*"
    allowed_until: "2029-12-31T23:59:59Z"
    approved_by: "sec@company.com"
    reason: "Migration"
`

	// Install pack to custom home
	origHome := os.Getenv("HOME")
	os.Setenv("HOME", tmp)
	defer os.Setenv("HOME", origHome)

	// Populate cache for offline scan to work
	...
	packDir := filepath.Join(tmp, ".pkgsafe", "policy-packs", "enterprise-standard", "2026.06.01")
	if err := os.MkdirAll(packDir, 0o755); err != nil {
		t.Fatal(err)
	}

	_ = os.WriteFile(filepath.Join(packDir, "metadata.json"), []byte(metaJSON), 0o644)
	_ = os.WriteFile(filepath.Join(packDir, "policy.yaml"), []byte(policyYAML), 0o644)
	_ = os.WriteFile(filepath.Join(packDir, "registries.yaml"), []byte(registriesYAML), 0o644)
	_ = os.WriteFile(filepath.Join(packDir, "trusted-packages.yaml"), []byte(trustedPackagesYAML), 0o644)
	_ = os.WriteFile(filepath.Join(packDir, "exceptions.yaml"), []byte(exceptionsYAML), 0o644)

	// Create a dummy package-lock.json with an npm package
	lockfilePath := filepath.Join(tmp, "package-lock.json")
	lockfileContent := `{
		"name": "fixture-app",
		"version": "1.0.0",
	...
	// 1. Run scan with enterprise mode enabled
	optsEnabled := ScanOptions{
		LockfilePath:   lockfilePath,
		PolicyPack:     "enterprise-standard",
		EnterpriseMode: true,
		Offline:        true,
	}

	resEnabled, err := RunScan(optsEnabled)
	if err != nil {
		t.Fatal(err)
	}

	if resEnabled.PolicyPack != "enterprise-standard" || resEnabled.PolicyPackVersion != "2026.06.01" {
		t.Errorf("expected PolicyPack 'enterprise-standard@2026.06.01', got '%s@%s'", resEnabled.PolicyPack, resEnabled.PolicyPackVersion)
	}

	if len(resEnabled.Findings) == 0 {
		t.Fatal("expected at least 1 finding")
	}
	f1 := resEnabled.Findings[0]
	if f1.Policy == nil || f1.Policy.Name != "enterprise-standard" {
		t.Errorf("expected finding policy evidence to be set when enterprise mode is enabled")
	}

	// 2. Run scan with enterprise mode disabled
	optsDisabled := ScanOptions{
		LockfilePath:   lockfilePath,
		PolicyPack:     "enterprise-standard",
	...
	resDisabled, err := RunScan(optsDisabled)
	if err != nil {
		t.Fatal(err)
	}

	if resDisabled.PolicyPack != "" || resDisabled.PolicyPackVersion != "" {
		t.Errorf("expected PolicyPack to be empty when enterprise mode is disabled")
	}

	if len(resDisabled.Findings) == 0 {
		t.Fatal("expected at least 1 finding")
	}
	f2 := resDisabled.Findings[0]
	if f2.Policy != nil {
		t.Errorf("expected finding policy evidence to be nil when enterprise mode is disabled")
	}
}
